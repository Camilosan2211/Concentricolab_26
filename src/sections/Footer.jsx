...
export default function Footer({ lang }) {
  ...
  return (
    <footer className="relative z-10 px-6 pb-8 pt-20 overflow-hidden">
      <div
        className="absolute top-0 left-1/2 -translate-x-1/2 w-[600px] h-px"
        style={{
          background: 'linear-gradient(to right,transparent,rgba(81,112,255,.22),transparent)',
        }}
      />
      ...
      <div className="max-w-[1200px] mx-auto">
        ...
        >
          <p className="text-[12px] font-bold tracking-[0.12em] uppercase dark:text-white/25 text-black/30">
        ...
        {/* Superior: navegación, productos, social */}
        <div className="grid grid-cols-2 md:grid-cols-3 gap-10 border-t dark:border-white/5 border-black/6 pt-10 pb-8">
          <div className="flex flex-col gap-3">
            <span className="text-[11px] font-bold tracking-[0.1em] uppercase dark:text-white/25 text-black/30">
        ...
        {/* Inferior */}
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 border-t dark:border-white/5 border-black/6 pt-8">
          <a href="#hero" className="inline-flex items-center gap-2 shrink-0 order-2 md:order-1 group">
            <img
              src="/assets/images/logo.png"
              alt=""
              className="h-6 w-auto opacity-95 dark:brightness-0 dark:invert"
            />
            <span className="footer-brand-glow font-cal text-sm font-semibold tracking-tight dark:text-white text-b-dark hidden sm:inline">
              Concéntrico Lab
            </span>
          </a>
          <p className="text-[11px] md:text-xs dark:text-white/22 text-black/30 text-center order-1 md:order-2 md:flex-1 md:px-4">
            © {year} ConcentricLab — {t('Bogotá D.C., Colombia', 'Bogotá D.C., Colombia')}
          </p>
          <p className="text-[11px] md:text-xs dark:text-white/22 text-black/30 text-center md:text-right order-3 md:max-w-[280px]">

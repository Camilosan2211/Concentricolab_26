  ...
  return(
    <section className="py-20 px-6">
      <div className="max-w-[1200px] mx-auto">
        <motion.div ref={ref}
          className="glass rounded-3xl p-8 md:p-14 grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-0 divide-y md:divide-y-0 md:divide-x dark:divide-white/6 divide-black/6 relative overflow-hidden"
          ...
          {stats.map((s,i)=>(
            <motion.div key={i} className="flex flex-col items-center text-center gap-3 px-6 py-6 md:py-0"
              initial={{opacity:0,y:20}} animate={inView?{opacity:1,y:0}:{}}
              transition={{delay:i*.18,duration:.8,ease:[.16,1,.3,1]}}>
              <motion.span className="font-cal text-5xl md:text-6xl font-bold"
                style={{color:s.color,textShadow:`0 0 36px ${s.color}50`}}
                initial={{scale:.8,opacity:0}} animate={inView?{scale:1,opacity:1}:{}}
                transition={{delay:.3+i*.18,type:'spring',stiffness:120}}>
                {s.num}
              </motion.span>
              <p className="dark:text-white/45 text-black/50 text-sm leading-[1.65] max-w-[180px]">
                {lang==='es'?s.es:s.en}
              </p>
            </motion.div>
          ))}

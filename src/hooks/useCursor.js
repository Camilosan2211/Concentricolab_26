...
export function useCursor() {
  useEffect(() => {
    ...
    loop()
    const ov = (e) => {
      if (e.target.closest('a,button,[data-mag]')) ring.classList.add('hov', 'lnk')
    }
    const ou = (e) => {
      if (e.target.closest('a,button,[data-mag]')) ring.classList.remove('hov', 'lnk')
    }
    const od = () => {
    ...
    return () => {
      ...
      document.removeEventListener('mousedown', od)
      document.removeEventListener('mouseup', ou2)
    }
  }, [])
}
